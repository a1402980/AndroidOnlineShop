...
public class MainActivity extends AppCompatActivity{
    ...
    private void generateData()
    {
        ...
        ItemEntity lenovo = new ItemEntity(UUID.randomUUID().toString(),"Lenovo Laptop", 565.00, "New laptop.", 5,  laptops.getUid(), false);
        ItemEntity hp = new ItemEntity(UUID.randomUUID().toString(),"HP Laptop", 450.00, "New laptop.", 4, laptops.getUid(), false);
        ItemEntity headphones = new ItemEntity(UUID.randomUUID().toString(),"JBL HeadPhones", 45.00, "New headphones.", 5,  accessories.getUid(), false);
        ItemEntity phonecase = new ItemEntity(UUID.randomUUID().toString(),"iphone 7 case", 15.00, "Phone Case for iphone 7", 3,  accessories.getUid(), false);
        ItemEntity panasonicTV = new ItemEntity(UUID.randomUUID().toString(),"Panasonic 55' 4k TV",899.00,"This TV is in great condition. I bought it one year ago",5,TVs.getUid(), false);
        ItemEntity iphone = new ItemEntity(UUID.randomUUID().toString(),"iphone 5s",399.00,"The phone is cracked a little bit but works just fine",2,phones.getUid(), false);
        ItemEntity hpprinter = new ItemEntity(UUID.randomUUID().toString(),"HP PRinter",49.95,"Used printer for 2 years and now I have a new one. Works just fine. Has some colors included.",4,printers.getUid(), false);
        ItemEntity asuslaptop = new ItemEntity(UUID.randomUUID().toString(),"Asus Laptop", 565.00, "New laptop.", 5,  laptops.getUid(), false);
        ItemEntity intelprocessor = new ItemEntity(UUID.randomUUID().toString(),"Intel processor i3", 52.95, "Intel processor i3. New.", 4, electronics.getUid(), false);
        ItemEntity simpleprinter = new ItemEntity(UUID.randomUUID().toString(),"Office Printer", 145.50, "New office printer.", 5,  printers.getUid(), false);
        ItemEntity samsungtablet = new ItemEntity(UUID.randomUUID().toString(),"Samsung note tablet", 155.00, "Used Samsung tablet", 3, tablets.getUid(), false);
        ItemEntity samsunggalaxy= new ItemEntity(UUID.randomUUID().toString(),"Samsung galaxy s7",399.00,"New Samsung Galaxy s7. Earphones and charger included",5,phones.getUid(), false);
        ItemEntity nexus5 = new ItemEntity(UUID.randomUUID().toString(),"Nexus 5X phone",299.00,"The phone is used but there are not scratches.",4,phones.getUid(), false);
        ...
        mDatabase.child("cart").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if(!dataSnapshot.exists())
                {
                    mDatabase.child("cart").child(cart.getUid()).setValue(cart);
                }
            }
            @Override
            public void onCancelled(DatabaseError databaseError) {
